...
public sealed class UnstoppableForceSkill : IBattleSkill
{
    ...
    public void Activate(in BattleEffectContext context, IBattleEffectSink effects)
    {
        ...
        foreach (BattleRuntimeUnit unitView in context.Units)
        {
            ...
            if (!BattleFieldSnapshot.IsValidEnemyTarget(casterState, unitState)) continue;

            if (Vector3.Distance(targetState.Position, unitState.Position) <= AreaRadius)
            {
                effects.DealDamage(new BattleDamageRequest { Source = casterState, Target = unitState, Amount = casterState.Attack * 2.5f, SourceKind = BattleEffectSourceKind.Skill, DamageKind = BattleDamageKind.Area, SkillId = SkillId, IsSkill = true, IsArea = true });
                effects.ApplyStatus(new BattleStatusRequest { Source = casterState, Target = unitState, Type = BattleStatusType.Stun, Level = 1, Duration = 1.5f, IsDebuff = true, IsDispelAllowed = true });
