...
public sealed class FreezeSkill : IBattleSkill
{
    ...
    public void Activate(in BattleEffectContext context, IBattleEffectSink contextEffects)
    {
        ...
        contextEffects.ScheduleEffect(8f, caster, caster, context, (ctx, sink) =>
        {
    ...
    private class FreezeAuraArtifact : IPositionHistoryArtifact, ISkillCastReactionArtifact
    {
        ...
        private readonly Dictionary<BattleRuntimeUnit, Vector3> _lastPositions = new Dictionary<BattleRuntimeUnit, Vector3>();
        private readonly Dictionary<BattleRuntimeUnit, float> _cooldowns = new Dictionary<BattleRuntimeUnit, float>();

        public FreezeAuraArtifact(float radius) { _radius = radius; }
        ...
        public void TickWithPositionHistory(BattleRuntimeUnit owner, BattlePositionHistory history, in BattleEffectContext context, IBattleEffectSink effects)
        {
            ...
            {
                if (unit == null || unit == owner || !BattleFieldSnapshot.IsValidEnemyTarget(owner.State, unit.State)) continue;
                if (Vector3.Distance(owner.Position, unit.Position) > _radius) continue;

                if (_cooldowns.TryGetValue(unit, out float cd) && cd > context.BattleTime) continue;

                if (_lastPositions.TryGetValue(unit, out Vector3 lastPos))
                {
                    if (Vector3.Distance(unit.Position, lastPos) > 0.1f)
                    {
                        effects.DealDamage(new BattleDamageRequest { Source = owner.State, Target = unit.State, Amount = owner.State.Attack * 1.5f, IsSkill = true });
        ...
        {
            if (evt.CasterView != null && BattleFieldSnapshot.IsValidEnemyTarget(owner, evt.Caster) && Vector3.Distance(owner.Position, evt.CasterView.Position) <= _radius)
            {
                effects.DealDamage(new BattleDamageRequest { Source = owner, Target = evt.Caster, Amount = owner.Attack * 2.0f, IsSkill = true });
