    ...
    public void Activate(in BattleEffectContext context, IBattleEffectSink effects)
    {
        ...
        foreach (var unit in context.Units)
        {
            ...
            if (Vector3.Distance(caster.Position, unit.Position) <= AreaRadius)
            {
                ...
                effects.ScheduleEffect(5f, caster, unit, context, (ctx, sink) =>
                {
                    if (unitVfx != null) VFXManager.Instance.StopEffect(unitVfx);
                });
            }
        }
        ...
        {
            if (activeZoneVfx != null) VFXManager.Instance.StopEffect(activeZoneVfx);
        });
    }
