...
public sealed class DuelSkill : IBattleSkill
{
    ...
    private class DuelArtifact : IDamageModifierArtifact
    {
        ...
        public DuelArtifact(BattleUnitCombatState duelTarget) { _duelTarget = duelTarget; }
        ...
        {
            if (request.Source == owner && request.Target == _duelTarget) request.Amount *= 1.50f;
            if (request.Target == owner && request.Source != _duelTarget) request.Amount *= 0.50f;
        }
    }
}
