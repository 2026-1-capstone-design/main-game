...
public sealed class DeathWhirlpoolSkill : IBattleSkill
{
    ...
    public IReadOnlyList<WeaponType> CompatibleWeaponTypes { get; } = new[] { WeaponType.handGun, WeaponType.dualGun, WeaponType.rifle };
    ...
    public void Activate(in BattleEffectContext context, IBattleEffectSink effects)
    {
        ...
        {
            float delay = i * 0.2f;
            effects.ScheduleEffect(delay, casterRuntime, casterRuntime, context, (ctx, sink) =>
            {
                ...
                foreach (BattleRuntimeUnit unitView in ctx.Units)
                {
                    ...
                    if (BattleFieldSnapshot.IsValidEnemyTarget(casterState, target) && Vector3.Distance(casterState.Position, target.Position) <= AreaRadius)
                    {
                ...
                if (validEnemies.Count > 0)
                {
                    ...
                    sink.DealDamage(new BattleDamageRequest { Source = casterState, Target = randomTarget, Amount = casterState.Attack * 0.4f, SourceKind = BattleEffectSourceKind.Skill, DamageKind = BattleDamageKind.Direct, SkillId = SkillId, IsSkill = true });
