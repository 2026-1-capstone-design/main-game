...
public sealed class ManaCollapseSkill : IBattleSkill
{
    ...
    private class ManaCollapseArtifact : IDamageReactionArtifact, ISkillCastReactionArtifact
    {
        ...
        {
            if (result.Source == null || result.Source.TeamId == owner.TeamId || result.Source == owner) return;
            ...
            effects.DealDamage(new BattleDamageRequest { Source = owner, Target = result.Source, Amount = dmg, SourceKind = BattleEffectSourceKind.Skill, DamageKind = BattleDamageKind.Direct, IsSkill = true });
        }

        public void OnSkillCast(BattleUnitCombatState owner, in BattleSkillCastEvent skillCastEvent, IBattleEffectSink effects)
        {
            if (skillCastEvent.Caster == null || skillCastEvent.Caster.TeamId == owner.TeamId) return;
            if (skillCastEvent.CasterView != null && Vector3.Distance(_ownerView.Position, skillCastEvent.CasterView.Position) <= _radius)
            {
                float dmg = skillCastEvent.Caster.MaxHealth * 0.10f;
                effects.DealDamage(new BattleDamageRequest { Source = owner, Target = skillCastEvent.Caster, Amount = dmg, SourceKind = BattleEffectSourceKind.Skill, DamageKind = BattleDamageKind.Direct, IsSkill = true });
            }
        }
    }
}
