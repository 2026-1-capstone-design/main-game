    ...
    public void Activate(in BattleEffectContext context, IBattleEffectSink effects)
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            foreach (var unit in context.Units)
            {
                ...
                if (unit == lastTarget || !BattleFieldSnapshot.IsValidEnemyTarget(casterState, unit?.State)) continue;
            ...
            if (nextTarget == null) break; // 더 이상 범위 내에 칠 수 있는 적이 없으면 연쇄 종료
        ...
        for (int i = 0; i < targetChain.Count; i++)
        {
            ...
            effects.ScheduleEffect(i * 0.2f, casterRuntime, target, context, (ctx, sink) =>
            {
                if (casterState.IsCombatDisabled) return; // 시전 중 사망 시 취소
                ...
                {
                    Source = casterState, Target = target.State,
