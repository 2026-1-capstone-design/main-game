...
- `GladiatorAgent.cs`
  ...
  - `WriteDiscreteActionMask()`에서 사망한 대상 슬롯과 사용할 수 없는 스킬 command를 막습니다.
  - `OnActionReceived()`에서 continuous 이동/회전과 discrete 전투 command를 `BattleRuntimeUnit`의 외부 제어 입력으로 변환합니다.
...
`Stage_1_smooth.yaml`은 BuiltInAI 상대 1v1~6v6 pretrain 전용입니다. 이 단계에서는 `self_play` 설정을 쓰지 않습니다. 이어서 Agent끼리 1v1~6v6 self-play를 진행할 때는 별도 trainer config를 사용합니다.
...
1. `TrainingScene`의 agent `BehaviorParameters`가 trainer YAML의 behavior name과 일치하는지 확인합니다.
   ...
   - action contract는 continuous 3개, discrete branch `[3, 6, 3]`, observation size `102`입니다.
