...
Observation/action contract가 바뀌면 기존 `.demo` 파일은 재사용할 수 없습니다. 현재 contract는 observation size `116`, continuous action `2`, discrete branch `[2, 6, 3]`입니다.
...
1. `TrainingScene`의 agent `BehaviorParameters`가 trainer YAML의 behavior name과 일치하는지 확인합니다.
   ...
   - action contract는 continuous 2개, discrete branch `[2, 6, 3]`, observation size `116`입니다.
...
3. curriculum YAML의 `team_size`와 `opponent_mode` lesson 순서를 함께 확인합니다.
