...
Observation/action contract가 바뀌면 기존 `.demo` 파일은 재사용할 수 없습니다. 현재 contract는 observation size `135`, continuous action `2`, discrete branch `[2, 4, 3, 4, 3, 6]`입니다.
...
1. `TrainingScene`의 agent `BehaviorParameters`가 trainer YAML의 behavior name과 일치하는지 확인합니다.
   - 현재 smooth contract의 behavior name은 `GladiatorSmooth`입니다.
   - action contract는 continuous 2개, discrete branch `[2, 4, 3, 4, 3, 6]`, observation size `135`입니다.
   - `Stage_1_smooth.yaml`은 BuiltInAI 상대 pretrain, `Stage_1_smooth_self_play.yaml`은 Agent끼리 self-play용입니다.
2. `TrainingBootstrapper.battleTicksPerEnvironmentStep`이 의도한 학습 계약인지 확인합니다. 기본값은 `1`입니다.
3. curriculum YAML의 `team_size`, `opponent_mode`, `lesson_mode` 순서를 함께 확인합니다.
   - `opponent_mode = 0.0`: PlayerTeam만 ML-Agent가 제어하고 HostileTeam은 기존 Plan/BuiltInAI로 행동합니다.
   - `opponent_mode = 1.0`: PlayerTeam과 HostileTeam 모두 ML-Agent가 제어합니다.
4. Editor 학습이면 `mlagents-learn ... --run-id=...`를 먼저 실행한 뒤 Unity Editor에서 Play합니다.
5. 빌드 학습이면 training scene이 포함된 executable을 만든 뒤 `--env`, `--num-envs`, `--no-graphics`를 사용합니다.
6. 학습 중에는 TensorBoard로 reward와 episode length를 확인합니다.
...
현재 기록되는 주요 메트릭은 다음과 같습니다.

| 메트릭 | 의미 |
|---|---|
| `Combat/AllyWin` | PlayerTeam이 승리한 episode 비율입니다. 0에 붙어 있으면 계속 패배 중입니다. |
| `Combat/BattleFinished` | timeout이나 interruption이 아니라 전투 승패로 종료된 episode 비율입니다. |
| `Combat/Timeout` | 제한 시간으로 종료된 episode 비율입니다. |
| `Combat/DamageDealtRatio` | episode 동안 대상 최대 체력 대비 누적 피해 비율입니다. 0에 가까우면 거의 못 때린 것입니다. |
| `Combat/DamageTakenRatio` | episode 동안 자기 최대 체력 대비 누적 피격 비율입니다. |
| `Combat/AttackIntent` | 사거리 안, 공격 가능, 후퇴 불필요 상태에서 기본공격을 선택한 횟수입니다. |
| `Combat/AttackLanded` | 실제 공격 명중 횟수입니다. |
| `Combat/AttackOpportunity` | 유효 타겟이 사거리 안에 있고 공격이 막히지 않았으며 후퇴 판단도 없는 공격 가능 decision 횟수입니다. |
| `Combat/AttackOpportunityUseRate` | `AttackOpportunity` 중 실제 기본공격을 선택한 비율입니다. 1에 가까울수록 기회를 거의 놓치지 않습니다. |
| `Combat/InRangeNoAttack` | 사거리 안, 공격 가능, 후퇴 불필요 상태에서 공격하지 않은 횟수입니다. |
| `Combat/OutOfRangeAttack` | 사거리 밖에서 기본공격을 선택한 횟수입니다. |
| `Combat/InvalidTargetAttack` | 유효 타겟 없이 기본공격을 선택한 횟수입니다. |
| `Combat/TargetSwitch` | episode 중 유효 타겟 슬롯을 바꾼 횟수입니다. |
| `Combat/MeanTargetDistance` | episode 중 유효 타겟까지의 평균 거리입니다. |
| `Combat/RoleEngageCompletionRate` | engage role 유지 완료 비율입니다. |
| `Combat/RolePeelCompletionRate` | peel role 유지 완료 비율입니다. |
| `Combat/RoleAssassinateCompletionRate` | assassinate role 유지 완료 비율입니다. |
| `Combat/RoleRegroupAbortRate` | regroup role 조기 이탈 비율입니다. |

1차 보상 디버깅에서는 먼저 아래 네 개를 함께 봅니다.
...
- `DamageDealtRatio`가 0에 가깝고 `MeanTargetDistance`가 높으면 접근 또는 타겟 추적 문제입니다.
- `AttackOpportunityUseRate`가 낮고 `InRangeNoAttack`이 높으면 공격 가능한 순간에 기본공격 선택을 못 배우는 문제입니다.
- `OutOfRangeAttack`이 높으면 접근 전에 헛공격을 누르는 정책입니다.
- `InvalidTargetAttack`이 높으면 타겟 action mask나 타겟 슬롯 관측을 먼저 확인합니다.
- `AttackIntent`는 높은데 `AttackLanded`가 낮으면 공격 입력과 실제 공격 판정/쿨타임/방향 전환 사이의 연결을 확인합니다.
