...
public sealed class BattleArtifactSystem : IBattleTargetingPolicy, IBattleMovementPolicy
{
    ...
    {
        if (owner == null || owner.State == null || artifact == null) return;

        artifact.Initialize(owner.State, context);

        if (artifact is IDamageModifierArtifact dmgMod) _damageModifiers.Add(new ArtifactBinding<IDamageModifierArtifact>(owner.State, owner, dmgMod));
        if (artifact is ITargetingModifierArtifact tgtMod) _targetingModifiers.Add(new ArtifactBinding<ITargetingModifierArtifact>(owner.State, owner, tgtMod));
        if (artifact is IMovementModifierArtifact movMod) _movementModifiers.Add(new ArtifactBinding<IMovementModifierArtifact>(owner.State, owner, movMod));
        if (artifact is IDamageReactionArtifact dmgReac) _damageReactions.Add(new ArtifactBinding<IDamageReactionArtifact>(owner.State, owner, dmgReac));
        if (artifact is IHealReactionArtifact healReac) _healReactions.Add(new ArtifactBinding<IHealReactionArtifact>(owner.State, owner, healReac));
        if (artifact is IKillReactionArtifact killReac) _killReactions.Add(new ArtifactBinding<IKillReactionArtifact>(owner.State, owner, killReac));
        if (artifact is ISkillCastReactionArtifact skillReac) _skillCastReactions.Add(new ArtifactBinding<ISkillCastReactionArtifact>(owner.State, owner, skillReac));
    }
    ...
    public void RemoveDynamicArtifact(IBattleArtifact artifact)
    {
        ...
        if (artifact is IDamageModifierArtifact) _damageModifiers.RemoveAll(b => b.Artifact == artifact);
        if (artifact is ITargetingModifierArtifact) _targetingModifiers.RemoveAll(b => b.Artifact == artifact);
        if (artifact is IMovementModifierArtifact) _movementModifiers.RemoveAll(b => b.Artifact == artifact);
        if (artifact is IDamageReactionArtifact) _damageReactions.RemoveAll(b => b.Artifact == artifact);
        if (artifact is IHealReactionArtifact) _healReactions.RemoveAll(b => b.Artifact == artifact);
        if (artifact is IKillReactionArtifact) _killReactions.RemoveAll(b => b.Artifact == artifact);
        if (artifact is ISkillCastReactionArtifact) _skillCastReactions.RemoveAll(b => b.Artifact == artifact);
    }
