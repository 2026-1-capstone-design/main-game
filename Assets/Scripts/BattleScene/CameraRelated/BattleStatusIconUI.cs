...
{
    [SerializeField] private Image iconImage;       // 버프/디버프 아이콘
    [SerializeField] private TextMeshProUGUI duration;
    [SerializeField] private TextMeshProUGUI levelText; // 중첩 수 (Level)
    [SerializeField] private GameObject debuffOutline; // 디버프일 경우 빨간 테두리 등 (선택사항)
    ...
    public void Setup(BattleStatusInstance status, Sprite iconSprite)
    {
        ...
        // 디버프 여부에 따라 테두리 활성화
        if (debuffOutline != null) debuffOutline.SetActive(status.IsDebuff);
    }
