...
기존 팀 단위 focus 비율은 같은 아군에게 몰린 압박과 분산된 압박을 구분하지 못해 전술 판단 신호로 쓰기 애매하다. 보호/peel 판단은 anchor 주변의 국지 관계 feature로 좁혀 표현한다.
...
| 아군 보조 판단 | 팀 단위 focus 비율 중심 | anchor 아군 단위 지원 가치 판단 가능 |
...
- `Assets/Scripts/BattleScene/Agent/GladiatorAgentActionParser.cs`
  — 역할 브랜치를 읽어 `GladiatorAction`에 반영
- `Assets/Scripts/BattleScene/Agent/Action/GladiatorAction.cs`
  — role 필드 추가
...
`GladiatorAction.cs`, `GladiatorAgentTacticalContext.cs`에 역할과 commitment 상태를 담을 필드를 추가한다.
